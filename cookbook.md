...
### Обсуждение
Само присутствие объявления *yield* в функции превращает её в генератор. В отличие от обычной функции, генератор запускается только в ответ на итерацию. Вот эксперимент, который вы можете провести, чтобы понять внутренний механизм работы таких функций:
```python
>>> def countdown(n):
...
